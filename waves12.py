...
tspan = [ts[0], ts[0]+10]
pre_n = solve_ivp(vecf, tspan, statevar[1,:], events= Vt_cross, method = 'RK45',rtol=1e-5,atol=1e-7)
...
tspan = [ts[1], ts[1]+10]
pre_n = solve_ivp(vecf, tspan, statevar[2,:], events= Vt_cross, method = 'RK45',rtol=1e-5,atol=1e-7)
