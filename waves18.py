...
def alphasyn(t,t_in,x):
	if (t>t_in and t_in>0):
		return Isyn[x]*np.exp(-(t-t_in)/tau2)
	else: return 0

def space_coupling(Isyn,x):
	return Isyn[x]
...
def vecfalpha(t, y0):
	...
	mNass=1./(1.+np.exp(-150.*(y0[0]+0.0305)))
	mK2ss=1./(1.+np.exp(-83.*(y0[0]+K2theta)))
	synapse1 = alphasyn(t,ts[0],0)
	syn_total = synapse1
		
	dv = (-1/Cm) * (gNa*mNass*mNass*mNass*y0[1]*(y0[0]-ENa)+ gK2*y0[3]*y0[3]*(y0[0]-EK) + gH*y0[2]*y0[2]*(y0[0]-EH) + gl*(y0[0]-El) + 0.006 +gsyn*syn_total*(y0[0]-Vrev))
	dh = (1/(1+ np.exp(500*(y0[0]+0.0325))) - y0[1])/tau_h
	dm = (1/(1+2*np.exp(180*(y0[0]+Htheta))+np.exp(500*(y0[0]+Htheta))) -y0[2])/tau_m
	dn = (mK2ss - y0[3])/tau_k
	f = dv, dh, dm, dn
...
def vecfalpha2(t, y0):
	...
	mNass=1./(1.+np.exp(-150.*(y0[0]+0.0305)))
	mK2ss=1./(1.+np.exp(-83.*(y0[0]+K2theta)))
	synapse1 = alphasyn(t,ts[0],1)
	synapse2 = alphasyn(t,ts[1],0)
	syn_total = synapse1+ synapse2
...
def vecfalpha3(t, y0):
	...
	mNass=1./(1.+np.exp(-150.*(y0[0]+0.0305)))
	mK2ss=1./(1.+np.exp(-83.*(y0[0]+K2theta)))
	# for control in np.arange(0,index,1):
	# 	synapsnap[0,control] = alphasyn(t,ts[control],control-control-1)
	# syn_total = np.sum(synapsnap)
	synapse1 = alphasyn(t,ts[0],2)
	synapse2 = alphasyn(t,ts[1],1)
	synapse3 = alphasyn(t,ts[2],0)
	syn_total = synapse1+synapse2+synapse3
...
ts[1] = neur1.t_events[0]
index += 1
print(index)
neur2 = solve_ivp(vecfalpha2, tspan_pre, y0,events= Vt_cross, method = 'RK45',rtol=1e-5,atol=1e-7)
ts[2] = neur2.t_events[0]
index += 1
print(index)
synapsnap = np.zeros((1,index))
neur3 = solve_ivp(vecfalpha3, tspan_pre, y0,events= Vt_cross, method = 'RK45',rtol=1e-5,atol=1e-7)
# ts[2] = neur3.t_events[0]

plt.ion()
plt.plot(pre_n.t,pre_n.y[0,:])
plt.plot(neur1.t,neur1.y[0,:])
plt.plot(neur2.t,neur2.y[0,:],marker='.')
plt.plot(neur3.t,neur3.y[0,:])